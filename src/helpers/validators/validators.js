...
export const requiredPhoto = (file) => {
    return file && Object.keys(file).length !== 0 ? undefined: 'This field is required'
};

export const weight = mb => photo => {
    return photo.size/Math.pow(10,6) < mb ? undefined: `Photo size cannot be more than ${mb} MB`
};

export const sizes = (minWidth, minHeight) => image => {
    return (image.height > minHeight && image.width > minWidth) ?
        undefined :
        `Invalid image, min width: ${minWidth}, min height: ${minHeight}`;
};

export const types = (typesImg) => image => {
    return typesImg.includes(image.type) ? undefined: `Image mime type must be ${typesImg}`
};
